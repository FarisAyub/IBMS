<?php
$ROOT = '../../'; 
require('../../connect.php'); // include the connection
include $ROOT.'nav.php'; // Include navigation bar
$username = $_SESSION['username']; // username of user logged in

// SQL Queries to get all database data
$result1 = mysqli_query($connect, "SELECT * FROM `ibms-Homeins`") or die(mysqli_error($connect));
$result2 = mysqli_query($connect, "SELECT * FROM `ibms-Healthins`") or die(mysqli_error($connect));
$result3 = mysqli_query($connect, "SELECT * FROM `ibms-Vehicleins`") or die(mysqli_error($connect));
$result4 = mysqli_query($connect, "SELECT * FROM `ibms-Accounts`") or die(mysqli_error($connect));
$result5 = mysqli_query($connect, "SELECT * FROM `ibms-Policies`") or die(mysqli_error($connect));
$result6 = mysqli_query($connect, "SELECT * FROM `ibms-Data`") or die(mysqli_error($connect));

if ($_SESSION['level'] == "2") { // Only show content if administrator
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="container">
    ...
    <div class="card card-dark">
      <div class="card-header">
        <!--- Tab definition for bootstrap tabs (Creates tabs that can be toggled to show different content), 1 for each table in database --->
      ...
      <div class="card-body">
        ...
        <script>
          $(document).ready(function() { // AJAX search that takes in key press and changes table contents to only show whats searched
        ...
        <div class="tab-content" id="policyTabContent">
          ...
          <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
            <p class="card-text text-center">
              <?php
              if (isset($_GET['updated'])) { // If details have been updated display alert
                $done = $_GET['updated'];
                if ($done == "true") {
              ...
              ?>
              <!-- Table containing all home insurance table details -->
              <table class='table table-bordered table-dark' style='font-size: 12px;'>
                <tr>
                ...
                <?php
                while ($row1 = mysqli_fetch_array($result1)) { // For all entries in table
                ?>
                <tbody id="filterable">
                  <tr>
                    ...
                    <td><?=$row1['Username']?></td>
                    <?php
                    switch ($row1['Status']) { // Check status and change colour of text 
                      case "Declined":
          ...
          <div class="tab-pane fade" id="health" role="tabpanel" aria-labelledby="health-tab">
            <p class="card-text text-center">
              <?php
              if (isset($_GET['updated'])) { // If a detail has been updated, display alert
                $done = $_GET['updated'];
                if ($done == "true") {
                  echo '<div class="alert alert-success alert-dismissible">';
                  echo '<button type="button" class="close" data-dismiss="alert">&times;</button>';
              ...
              ?>
              <!-- Table containing all health insurance records -->
              <table class='table table-bordered table-dark' style='font-size: 12px;'>
                <tr>
                  <td width="3%"><b>ID</b></td>
                  <td><b>Username</b></td>
                ...
                <?php
                while ($row2 = mysqli_fetch_array($result2)) { // For each record
                ?>
                <tbody id="filterable">
                  <tr>
                    ...
                    <td><?=$row2['Username']?></td>
                    <?php
                    switch ($row2['Status']) { // Check status and change colour of text
                      case "Declined":
          ...
          <div class="tab-pane fade" id="vehicle" role="tabpanel" aria-labelledby="vehicle-tab">
            <p class="card-text text-center">
              <?php
              if (isset($_GET['updated'])) { // If a change has been made, display alert
                $done = $_GET['updated'];
                if ($done == "true") {
                  echo '<div class="alert alert-success alert-dismissible">';
                  echo '<button type="button" class="close" data-dismiss="alert">&times;</button>';
              ...
              ?>
              <!--- Table containing all vehicle insurance from database -->
              <table class='table table-bordered table-dark' style='font-size: 12px;'>
                <tr>
                  <td width="3%"><b>ID</b></td>
                  <td><b>Username</b></td>
                ...
                <?php
                while ($row3 = mysqli_fetch_array($result3)) { // For all rows in the database
                ?>
                <tbody id="filterable">
                  <tr>
                    ...
                    <td><?=$row3['Username']?></td>
                    <?php
                    switch ($row3['Status']) { // Check status and change colour of text
                      case "Declined":
          ...
          <div class="tab-pane fade" id="account" role="tabpanel" aria-labelledby="account-tab">
            <p class="card-text text-center">
              ...
              ?>
              <!-- Table containing all account details -->
              <table class='table table-bordered table-dark' style='font-size: 12px;'>
                <tr>
                  <td><b>Username</b></td>
                  <td><b>Password</b></td>
                ...
                <?php
                while ($row4 = mysqli_fetch_array($result4)) { // For all rows in database for account details
                ?>
                <tbody id="filterable">
                  <tr>
                    <td><?=$row4['Username']?></td>
          ...
          <div class="tab-pane" id="policies" role="tabpanel" aria-labelledby="policies-tab">
            <p class="card-text text-center">
              ...
              ?>
              <!-- Table containing all policies stored -->
              <table class='table table-bordered table-dark' style='font-size: 12px;'>
                <tr>
                  <td width="3%"><b>ID</b></td>
                ...
                <?php
                while ($row5 = mysqli_fetch_array($result5)) { // For each policy in the database
                ?>
                <tbody id="filterable">
                  <tr>
          ...
          <div class="tab-pane fade" id="data" role="tabpanel" aria-labelledby="data-tab">
            <p class="card-text text-center">
              <?php
              if (isset($_GET['updated'])) { // If a change was made, display alert 
                $done = $_GET['updated'];
                if ($done == "true") {
                  echo '<div class="alert alert-success alert-dismissible">';
                  echo '<button type="button" class="close" data-dismiss="alert">&times;</button>';
              ...
              ?>
              <!-- Table containing all policy details -->
              <table class='table table-bordered table-dark' style='font-size: 12px;'>
                <tr>
                  <td><b>Policy_id</b></td>
                  <td><b>Views</b></td>
                ...
                <?php
                while ($row6 = mysqli_fetch_array($result6)) { // For each policy in data table
                ?>
                <tbody id="filterable">
                  <tr>
                    <td><?=$row6['Policy_id']?></td>
                    <td><?=$row6['Views']?></td>
                    <td><?=$row6['Likes']?></td>
                    <td><a href="edit.php?id=<?=$row6['Policy_id']?>&frm=x" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt"></i></a></td>
                  </tr>
                </tbody>
                <?php } ?>
              </table>
